...
<h3 class="text-center">Games: {{$games->title}}</h3>
<h3 class="text-center">Score: {{$scores->score}}</h3>

    <br>
    <a href="{{route('scores.edit',$scores->id)}}" class="btn btn-primary float-left">Update</a>
    ...
    <form method="POST" id="delete-form" action="{{route('scores.destroy',$scores->id)}}">
        @csrf
        @method('DELETE')
    </form>
